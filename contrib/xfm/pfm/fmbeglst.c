...
*
*  Modified: 20 Nov 1992 - To scoot over only current textsize + a few.
...
void FMbeglst( )
{
     ...
     if( toupper( buf[0] ) == 'F'  && buf[1] == '=' )  /* font entered */
      {
       ...
       strncpy( new->font, buf+2, len );              /* copy in the string */
       new->font[len] = EOS;                        /* mark its end */
      }
     else
 ...
 lmar += new->size + 3;       /* scoot over where text will be placed */
 linelen -= new->size + 3;    /* reduce number of points per line */
