...
void FMbxstart( int option, char *colour, int border, int width, char *align )
{
 ...
 if( option )        /* FALSE option should not occur in htfm, but in case...*/
  {
   sprintf( outb, "<center> <table width=%d%% %s border=%d %s>", width, align, border, colour );
   AFIwrite( ofile, outb );
   sprintf( outb, "<tr><td valign=top>" );
   AFIwrite( ofile, outb );
  }
}                       /* FMbxstart */
