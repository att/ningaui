/*
	changes for ningaui:
		06 Feb 2007 (sd) : to avoid what seems to be calloc() problems
			in 64 bit, i386, FreeBSD environment, the single calloc()
			call was converted to a malloc/memset pair. 
*/
...
shifttab(register Node *n)
{
	...
	n->tab = nn = (Node **)malloc(256 * sizeof(Node *));
	if( ! nn )
	{
		fprintf( stderr, "shifttab: malloc failed: %s\n", strerror( errno ) );
		exit( 1 );
	}
	memset( nn, 0, 256 * sizeof( Node *) );
	for(l = n->alts; l; l = l->next)
		nn[l->l] = l->node;
}
