/* 
	version 02067 -- removed calloc() calls in some modules to avoid what seems to 
		be a problem with calloc on 64bit, i86, FreeBSD environments.
*/
...
/*re_debug=20;/**/
	...
	*/
	...
	if(cflag && (lflag|Lflag)){
		EPR "%s: cannot use -c with -l or -L\n", progname);
		exit(2);
	}
	/*
		character mapping ?
