...
typedef ng_uint32 UINT4;
#define	elementsof(x)	sizeof(x)
...
md5_init(void)
{
	register Md5_t*	context;

	context = ng_malloc(sizeof(*context), "md5_t");
	...
	memset(context->digest_sum, 0, sizeof(context->digest_sum));
	return context;
}
...
void
md5_block(Md5_t* context, const void* s, size_t inputLen)
{
...
md5_done(Md5_t*	context)
{
	...
	md5_block(context, md5_pad, padLen);

	/* append length (before padding) */
	md5_block(context, bits, sizeof(bits));
...
void
md5_sum(Md5_t *x, char *buf)
{
	register unsigned char*	d;
	register int		n;
	int p;
	int	flags = 1;	/* SUM_TOTAL or not */
	char	*hex = "0123456789abcdef";

	d = flags ? x->digest_sum : x->digest;
	p = 0;
